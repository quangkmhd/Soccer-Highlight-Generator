...
class Model(nn.Module):
    def __init__(self, weights=None, input_size=512, num_classes_type=13,num_classes_change=1, chunk_size=240, dim_capsule=16, receptive_field=80, num_detections=5, framerate=2):
        """
        ...
        self.conv_seg = nn.Conv2d(in_channels=152, out_channels=dim_capsule*num_classes_type, kernel_size=(self.kernel_seg_size,1))
        self.batch_seg = nn.BatchNorm2d(num_features=self.chunk_size) # Dimension on which to perform it to be checked
        ...
        self.conv_spot_1 = nn.Conv2d(in_channels=num_classes_type*(dim_capsule+1), out_channels=32, kernel_size=(self.kernel_spot_size,1))
        ...
        # Class branch
        # self.conv_class = nn.Conv2d(in_channels=16*(chunk_size//8-1), out_channels=self.num_detections*self.num_classes_change, kernel_size=(1,1))
    ...
    def forward(self, inputs):
        ...
        conv_seg_reshaped = conv_seg_permuted.view(conv_seg_permuted.size()[0],conv_seg_permuted.size()[1],self.dim_capsule,self.num_classes_type)
        ...
        #output_segmentation = torch.sigmoid(torch.sum(conv_seg_reshaped, dim=2))
        output_segmentation = F.softmax(torch.sum(conv_seg_reshaped, dim=2), dim=-1)
        ...
        output_segmentation_reverse = output_segmentation
        ...
        # Class branch
        #conf_class = self.softmax(self.conv_class(spotting_reshaped).view(spotting_reshaped.shape[0],self.num_detections,self.num_classes_change))
        #print("Conf_class size: ", conf_class.size())

        #output_spotting = torch.cat((conf_pred,conf_class),dim=-1)
        output_spotting = conf_pred
