...
def main(args, model_save_path):
    ...
    if not args.test_only:
        ...
        dataset_Valid_metric = SoccerNetClipsTesting(path=args.SoccerNet_path, features=args.features, split="valid", version=args.version, framerate=args.framerate, chunk_size=args.chunk_size*args.framerate, receptive_field=args.receptive_field*args.framerate)
    ...
    if not args.test_only:
		...
		advanced_test=args.advanced_test)

    best_model_path = ""
    if args.loss_weight_segmentation == 0 or args.loss_weight_detection == 0:
        best_model_path = os.path.join(model_save_path, "model_best_eval.pth.tar")
    else:
        # best_model_path = os.path.join(model_save_path, "model.pth.tar")
        best_model_path = os.path.join(model_save_path, "model_best_loss.pth.tar")
    ...
    if os.path.exists(best_model_path):
        ...
        performance_test= test(test_loader, model, "best",  model_save_path)
        log_performance(performance_test, -1, f"Best test performance", args.advanced_test)
        # logging.info("Best Test Performance " + str(performance_test))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_epochs',   required=False, type=int,   default=200,     help='Maximum number of epochs' )
    ...
    parser.add_argument('--evaluation_frequency', required=False, type=int,   default=20,     help='How frequently to evaluate performance metrics on the validation set' )
    ...
    parser.add_argument('--chunk_size', required=False, type=int,   default=24,     help='Size of the chunk (in seconds)' )
    parser.add_argument('--receptive_field', required=False, type=int,   default=8,     help='Temporal receptive field of the network (in seconds)' )
    parser.add_argument('--num_detections', required=False, type=int,   default=9,     help='Maximal number of detections per chunk' )
    ...
    parser.add_argument("--loss_weight_segmentation", required=False, type=float, default=0.8, help="Weight of the segmentation loss compared to the detection loss")
