...
class SegmentationLoss(torch.nn.Module):
    ...
    def forward(self, gt_label, pred_score):
        # pred_score shape: torch.Size([32, 48, 13]) # Batch_size, Chunk_size, num_classes

        pred_score = torch.clamp(pred_score, 10e-3, 1 - 10e-3)
...
#            sum_loss=sum_loss+temp_loss
        ...
        loss = self.cross_entropy_no_softmax(gt_label, pred_score)
        return loss
    ...
    def cross_entropy_no_softmax(self, gt_label, pred_score):
        loss_elementwise = -gt_label * torch.log(pred_score) - (1 - gt_label) * torch.log(1 - pred_score)
        loss_summed = torch.sum(loss_elementwise, dim=1)
        loss_summed = torch.sum(loss_summed, dim=0) * self.weight
        loss_summed = torch.sum(loss_summed)
        return loss_summed
